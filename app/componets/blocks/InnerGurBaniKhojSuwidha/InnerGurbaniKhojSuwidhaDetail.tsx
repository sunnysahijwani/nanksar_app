...
export default function InnerGurbaniKhojSuwidhaDetail({ route }: any) {
    ...
    const { data, isLoading } = useGuruGranthSahibjiBani(currentPageIndex, currentPageIndex + 5);
    ...
    useFocusEffect(
        useCallback(() => {
            const getMyData = (data: any) => {
                ...
                let currentData = data?.[`${page_index_prefix + currentPageIndex}`] || [];
                if (!currentData || !currentData?.length) {
                    currentData = data?.[`${page_index_prefix + (currentPageIndex - 1)}`] || [];
                };
                setLoading(false);
                return currentData;
            }
        ...
        <>
            <GradientBg colorsList={["#ffffff", "#ffffff"]}>
                <View className='flex-1'>
                    <View>
                        <AudioListingHeader handleSettingsPress={handleSettingsPress} />
                    </View>

                    {/* main content ares  */}
                    <ScrollView className='flex-1'>
                        <View className='items-center justify-center'
                            style={{ paddingHorizontal: SIZES.screenDefaultPadding, paddingVertical: SIZES.screenDefaultPadding, gap: SIZES.xsSmall }}>
